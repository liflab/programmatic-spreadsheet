 ...
 */
public class DrawChart extends AtomicFunction
{
	/**
	 * The plot object used to produce the image.
	 */
	/*@ non_null @*/ protected final Chart m_plot;

	/**
	 * The image format to produce.
	 */
	/*@ non_null @*/ protected ChartFormat m_format;
	 ...
	 */
	public DrawChart(/*@ non_null @*/ Chart p, ChartFormat format, boolean with_title)
	{
		super(1, 1);
		m_plot = p;
		m_format = format;
	 ...
	 * @param p The plot object used to produce the image
	 */
	public DrawChart(/*@ non_null @*/ Chart p)
	{
		this(p, null, true);
	}
	 ...
	 * part of a {@link Chart} is replaced by a reference to the first input of
	 * the function.
	 */
		...
		protected void visitLeaf(Node n)
		{
			if (n instanceof PartNode)
			{
				...
				if (!m_visited.contains(pn))
				{
					...
					PartNode new_leaf = m_factory.getPartNode(new_p, DrawChart.this);
